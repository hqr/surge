...
type TioInterface interface {
	...
	GetTioChild(tgt NodeRunnerInterface) TioInterface
	addTioChild(tiochild TioInterface)
	hasChildren() bool
	GetNumTioChildren() int
	DelTioChild(tgt NodeRunnerInterface)
	GetRepnum() int
	IsInit() bool
	Done() bool
	RemoveWhenDone() bool
	...
	// real stuff
	nextAnon(when time.Duration, tgt NodeRunnerInterface)
	next(newev EventInterface, sendhow SendMethodEnum)
	doStage(r NodeRunnerInterface, args ...interface{}) error
	abort()
	String() string
}
...
type Tio struct {
	...
	parent         TioInterface
	flow           FlowInterface
	children       map[NodeRunnerInterface]TioInterface
...
//
//================================================================================
func NewTio(src NodeRunnerInterface, p *Pipeline, args ...interface{}) *Tio {
	assert(p.Count() > 0)
...
func (tio *Tio) setOneArg(a interface{}) {
	...
	case NodeRunnerInterface:
		tio.target = a.(NodeRunnerInterface)
	case FlowInterface:
...
func NewTioRr(src NodeRunnerInterface, p *Pipeline, args ...interface{}) *TioRr {
	uqid, printid := uqrandom64(src.GetID())
	tio := &Tio{
		id:             uqid,
		sid:            printid,
...
func NewTioOffset(src NodeRunnerInterface, p *Pipeline, args ...interface{}) *TioOffset {
	uqid, printid := uqrandom64(src.GetID())
	tio := &Tio{
		id:             uqid,
		sid:            printid,
...
func (tio *Tio) GetSource() NodeRunnerInterface                   { return tio.source }
func (tio *Tio) GetChunkSid() int64                               { return tio.chunksid }
func (tio *Tio) GetTarget() NodeRunnerInterface                   { return tio.target }
...
func (tio *Tio) GetTioChild(tgt NodeRunnerInterface) TioInterface { return tio.children[tgt].GetTio() }

func (tio *Tio) GetNumTioChildren() int              { return len(tio.children) }
func (tio *Tio) DelTioChild(tgt NodeRunnerInterface) { delete(tio.children, tgt) }
...
func (tioparent *Tio) addTioChild(tiochild TioInterface) {
	if tioparent.children == nil {
		tioparent.children = make(map[NodeRunnerInterface]TioInterface, 2)
	}
...
func (tio *Tio) nextAnon(when time.Duration, tgt NodeRunnerInterface) {
...
func (tio *Tio) doStage(r NodeRunnerInterface, args ...interface{}) error {
	tioevent := tio.event
	if len(args) > 0 {
